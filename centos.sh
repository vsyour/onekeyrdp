...
sudo su - $userName -c "sudo yum update -y "
sudo su - $userName -c "sudo yum upgrade -y"
sudo su - $userName -c "sudo yum install -y epel-release"
sudo su - $userName -c "sudo yum groupinstall -y 'X window system'"
sudo su - $userName -c "sudo yum --enablerepo=epel group -y install 'Xfce' 'base-x'"
echo "xfce4-session" > /home/"${userName}"/.Xclients
chmod a+x /home/"${userName}"/.Xclients	
sudo systemctl set-default graphical.target
sudo yum install -y firefox
sudo yum install wqy* cjkuni* -y
sudo yum locales ttf-wqy-zenhei ttf-wqy-microhei
