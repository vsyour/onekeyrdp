...
__addUser(){
	grep "${userName}" /etc/passwd || sudo useradd -m "${userName}" && sudo adduser "${userName}" sudo && echo "${userName}:${passWord}" | sudo chpasswd && sed -i "/${userName}:x:1000:1000::\/home\/${userName}:\/bin\/sh/d" /etc/passwd && echo "${userName}:x:1000:1000::/home/${userName}:/bin/bash" >> /etc/passwd
}
