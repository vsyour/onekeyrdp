...
userName='debian'
passWord=`< /dev/urandom tr -dc _A-Z-a-z-0-9 | head -c8;echo`
...
__addUser(){
	grep "${userName}" /etc/passwd || sudo useradd -m "${userName}" && echo "${userName}  ALL=(ALL) NOPASSWD:ALL" | sudo tee "/etc/sudoers.d/${userName}" | sudo chpasswd && sed -i "/${userName}:x:1000:1000::\/home\/${userName}:\/bin\/sh/d" /etc/passwd && echo "${userName}:x:1000:1000::/home/${userName}:/bin/bash" >> /etc/passwd
}
...
__installSoftware(){
	sudo $PM firefox-esr locales ttf-wqy-zenhei ttf-wqy-microhei
