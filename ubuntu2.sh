...
sudo su - $userName -c "sudo apt -y install xfce4 lxterminal mousepad < /dev/null > /dev/null"

date "+【%Y-%m-%d %H:%M:%S】 Install xrdp." 2>&1 | tee -a $logPath
sudo su - $userName -c "sudo apt -y install xrdp tigervnc-standalone-server"
